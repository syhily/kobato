import type { MusicPlayerNode } from '@kobato/shared/lexical/nodes/music-player-node'
...
/** Editor-local music-player payload — the PT `musicPlayer` shape the node fields map to. */
interface EditorMusicPlayerPayload {
  _type: 'musicPlayer'
  _key: string
  playerId: string
  auto?: boolean
...
export function MusicPlayerView({ node, editor }: MusicPlayerViewProps) {
  ...
  const payload: EditorMusicPlayerPayload = {
    _type: 'musicPlayer',
