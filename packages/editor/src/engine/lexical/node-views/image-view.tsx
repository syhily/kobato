import type { ImageNode } from '@kobato/shared/lexical/nodes/image-node'
...
/** Editor-local layout vocabulary (mirrors the node dialect's `layout` field). */
type EditorImageLayout = 'left' | 'center' | 'right'
...
export function ImageView({ node, editor }: ImageViewProps) {
  ...
  const resolvedLayout: EditorImageLayout = layout === 'left' || layout === 'right' ? layout : 'center'

  const setLayout = (next: EditorImageLayout) => {
    editor.update(() => {
