import type { MathBlockNode } from '@kobato/shared/lexical/nodes/math-block-node'
...
/** Editor-local math-block payload — the PT `mathBlock` shape the node fields map to. */
interface EditorMathBlockPayload {
  _type: 'mathBlock'
  _key: string
  tex: string
  mathml?: string
...
/** Minimal block envelope the block-card `onCommit` may hand back. */
interface EditorBlockShape {
  _type: string
  [key: string]: unknown
}
...
export function MathBlockView({ node, editor }: MathBlockViewProps) {
  ...
  const payload: EditorMathBlockPayload = {
    _type: 'mathBlock',
  ...
  const commitPayload = (next: EditorBlockShape, editorRender?: string) => {
    if (next._type !== 'mathBlock') {
      return
    }
    // The discriminant guard narrows the runtime shape; the envelope's
    // index signature cannot express the literal fields, so cast through
    // the payload interface.
    // eslint-disable-next-line ts/no-unsafe-type-assertion
    const mathBlock = next as unknown as EditorMathBlockPayload
    editor.update(() => {
      node.setTex(mathBlock.tex)
      node.setMathml(editorRender !== undefined && editorRender !== '' ? editorRender : mathBlock.mathml)
    })
    setEditing(false)
  }
