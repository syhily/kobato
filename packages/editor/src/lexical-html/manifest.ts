...
//
// Values are byte-identical to the PT render adapters they replace
// (`@kobato/editor/renderer/render-shared.ts` PT_INLINE, the block
// classNames in `render-blocks.tsx` / `render-marks.tsx`, and the
// server feed forms in `@kobato/server/render/pt-html.ts`). The PT
// track stays the owner until it is retired (dual-track period); this
// manifest is the lexical-side copy. Footnote anchor ids / hrefs are
// owned by `@kobato/shared/lexical/footnote-anchors` and are NOT
// duplicated here.
//
// Mode contract:
//   - `default` — full-class form, identical to the React renderer
//   - `rss`     — classless degraded form (mirrors pt-html.ts RSS)
...
// `DARK_IMAGE_DIM_CLASS` (`engine/widgets/Image.tsx`) — copied here so
...
/** Feed/string placeholder when metadata is missing (`pt-html.ts`). */
