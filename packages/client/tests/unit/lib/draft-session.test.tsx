...
vi.mock('@kobato/client/lib/draft-store', () => ({
...
import type { DraftRecord } from '@kobato/client/lib/draft-store'
...
} from '@kobato/client/lib/draft-session'
import { EMPTY_LEXICAL_BODY, lexicalBodySchema, type LexicalBody } from '@kobato/shared/lexical/schema'
...
type Loaded = { body: LexicalBody; savedAt: number }

function mapLoaded(record: DraftRecord, parsedBody: LexicalBody): Loaded {
  return { body: parsedBody, savedAt: record.savedAt }
}

function makeArgs(
  overrides: Partial<UseDraftSessionArgs<LexicalBody, Loaded>> = {},
): UseDraftSessionArgs<LexicalBody, Loaded> {
  return {
    key: KEY,
...
describe('useDraftSession — load and validate-or-purge', () => {
  it('hydrates a valid stored draft through mapLoaded', async () => {
    ...
    expect(result.current.loadedDraft).toEqual({ body: EMPTY_LEXICAL_BODY, savedAt: 123 })
  ...
  it('purges a record whose body fails the schema', async () => {
    draftStore.get.mockResolvedValue(storedRecord({ body: 'not-lexical' }))
    const { result } = renderHook(() => useDraftSession(makeArgs()))
    await flushDraftEffects()
    expect(draftStore.remove).toHaveBeenCalledWith(KEY)
    expect(result.current.loadedDraft).toBeNull()
  ...
  it('re-reads when the key changes, at most once per key', async () => {
    ...
    await flushDraftEffects()
    expect(result.current.loadedDraft).toEqual({ body: EMPTY_LEXICAL_BODY, savedAt: 1 })

    rerender({ key: 'b' })
    await flushDraftEffects()
    expect(draftStore.get).toHaveBeenCalledWith('b')
    expect(result.current.loadedDraft).toEqual({ body: EMPTY_LEXICAL_BODY, savedAt: 2 })
...
describe('useDraftSession — persist gating', () => {
  it('never writes before the initial load completes', async () => {
    const pendingBody = ['pending'] as unknown as LexicalBody
    const settledBody = ['settled'] as unknown as LexicalBody
    ...
    const { rerender } = renderHook(({ body }) => useDraftSession(makeArgs({ body })), {
      initialProps: { body: EMPTY_LEXICAL_BODY },
    })
  ...
  it('persists meta alongside the body when provided', async () => {
    ...
    const changedBody = ['changed'] as unknown as LexicalBody
...
describe('useDraftSession — inactive key', () => {
  ...
  it('starts the session when the key becomes non-null', async () => {
    ...
    await flushDraftEffects()
    expect(draftStore.get).toHaveBeenCalledWith(KEY)
    expect(result.current.loadedDraft).toEqual({ body: EMPTY_LEXICAL_BODY, savedAt: 9 })
  })
})
