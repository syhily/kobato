...
test.describe('Image card', () => {
    ...
    await assertHTML(
      ...
      html`
        <div data-lexical-decorator="true" contenteditable="false" data-inkling-card-width="wide">
          <div data-inkling-card-editing="false" data-inkling-card-selected="false" data-inkling-card="image">
            <figure data-inkling-card-width="wide">
              <div><img alt="" src="/content/images/2022/11/inkling-lexical.jpg" /></div>
              <figcaption>
  ...
  test('can get image dimensions from external URL', async function () {
    await page.route('https://example.com/large-image.png', (route) =>
      route.fulfill({
        status: 200,
  ...
  test('can toggle to alt text', async function () {
    ...
    // wait for upload to complete and card to be selected with toolbar visible
        ...
        <div data-lexical-decorator="true" contenteditable="false">
          <div data-inkling-card-editing="false" data-inkling-card-selected="true" data-inkling-card="image">
            <figure data-inkling-card-width="regular">
              ...
              <figcaption>
                <input placeholder="Type alt text for image (optional)" type="text" value="" />
  ...
  test('renders caption if present', async function () {
    ...
    await expect(page.locator('text="This is a caption"')).toBeVisible()
  })
        ...
        <div data-lexical-decorator="true" contenteditable="false">
          <div data-inkling-card-editing="false" data-inkling-card-selected="true" data-inkling-card="image">
            <figure data-inkling-card-width="regular">
              <div><img alt="" src="blob:..." /></div>
              <figcaption>
                <div data-testid="image-caption-editor" data-inkling-allow-clickthrough="true">
                  <div>
                    <div data-inkling="editor">
                      <div
                      ...
                      >
                        <p dir="ltr" data-inkling-dnd-droppable="true">
  ...
  test('can handle drag over & leave', async function () {
    ...
    await expect(page.locator('[data-inkling-card-drag-text="true"]')).toHaveCount(0)
  })
  ...
  test('can handle image drop on empty card', async function () {
    ...
    // wait for upload to complete
    await expect(page.getByTestId('progress-bar')).toBeHidden()
    await expect(page.locator('[data-inkling-card="image"] img')).toHaveAttribute('alt', '')
  ...
  test('replaces image when new image file dropped on populated card', async function () {
    ...
    const originalSrc = await page.locator('[data-inkling-card="image"] img').getAttribute('src')

    const filePath = fixture('large-image-1.png')
    const dataTransfer = await createDataTransfer(page, [
      { filePath, fileName: 'large-image-1.png', fileType: 'image/png' },
    ])

    await page
      .locator('[data-inkling-card="image"] [data-testid="image-card-populated"]')
      .dispatchEvent('dragenter', { dataTransfer })

    // Dragover text should be visible
    await expect(page.locator('[data-inkling-card="image"] [data-testid="drag-overlay"]')).toBeVisible()
    ...
    const newSrc = await page.locator('[data-inkling-card="image"] img').getAttribute('src')
  ...
  test('can insert tenor image', async () => {
    ...
    await expect(page.locator('[data-gif-index="1"]')).toBeVisible()
      ...
      html`
        <div data-lexical-decorator="true" contenteditable="false">
          <div data-inkling-card-editing="false" data-inkling-card-selected="true" data-inkling-card="image">
            <figure data-inkling-card-width="regular">
              <div>
                <img alt="" src="https://media.tenor.com/ocbMLlwniWQAAAAC/steve-harvey-oh.gif" />
              </div>
              <figcaption>
                <div data-testid="image-caption-editor" data-inkling-allow-clickthrough="true">
                  <div>
  ...
  test('can insert a gif with the keyboard', async () => {
    ...
    await expect(page.locator('[data-testid="gif-item"]')).toHaveCount(3)
    await expect(page.locator('[data-gif-index="2"]')).toBeVisible()
      ...
      html`
        <div data-lexical-decorator="true" contenteditable="false">
          <div data-inkling-card-editing="false" data-inkling-card-selected="true" data-inkling-card="image">
            <figure data-inkling-card-width="regular">
              <div>
                <img alt="" src="https://media.tenor.com/Sm9aylrzSyMAAAAC/cats-animals.gif" />
              </div>
              <figcaption>
                <div data-testid="image-caption-editor" data-inkling-allow-clickthrough="true">
                  <div>
  ...
  test('can close the gif selector on Esc', async () => {
    ...
    await expect(page.getByTestId('gif-selector')).toBeVisible()
  ...
  test('can show a gif selector error', async () => {
    ...
    await expect(page.getByTestId('gif-selector-error')).toBeVisible()
  })
  ...
  test('can add snippet', async function () {
    ...
    await page.keyboard.press('Escape')
    await expect(page.locator('[data-inkling-card="image"]')).toHaveAttribute('data-inkling-card-editing', 'false')
  ...
  test('can select caption text without scrolling', async function () {
    ...
    const rootParagraphs = page.locator('.inkling-lexical > [data-inkling="editor"] > div > p')
    const paragraphCount = await rootParagraphs.count()

    await page.keyboard.type('Captiontest--Captiontest')

    const captionEditor = page.locator('[data-testid="image-caption-editor"] [data-inkling="editor"] p span')
    ...
    await page.evaluate(() => {
      const caption = document.querySelector('[data-testid="image-caption-editor"]')
      if (!caption) {
        throw new Error('Expected image caption editor')
      }
    ...
    await expectUnchangedScrollPosition(page, async () => {
      ...
      const box = await getBoundingBox(captionEditor)
      ...
      await expect(page.locator('[data-inkling-card="image"]')).toHaveAttribute('data-inkling-card-selected', 'false')
  ...
  test('can select caption text and make it italic', async function () {
    ...
    await expectUnchangedScrollPosition(page, async () => {
      ...
      // Click italic button
      await page.locator('[data-inkling-toolbar-button="italic"]').click()

      // Check contains text
      await expect(captionEditor).toHaveText('-Captiontest')
      const italicSpan = page.locator('[data-testid="image-caption-editor"] [data-inkling="editor"] p em').nth(0)
  ...
  test.describe('image should be a top-level element', () => {
    test('can insert image to nested list', async function () {
      ...
      await expect(page.locator('ul:has([data-inkling-card="image"])')).toHaveCount(0)
    })
    ...
    test('can paste image with text from google doc', async function () {
      ...
      await pasteHtml(
        page,
        '<meta charset=\'utf-8\'><meta charset="utf-8"><b style="font-weight:normal;" id="docs-internal-guid-338a96ba-7fff-6ecc-7aac-61f170b38b59"><p dir="ltr" style="line-height:1.38;margin-top:12pt;margin-bottom:12pt;"><span style="font-size:12pt;font-family:Arial;color:#000000;background-color:transparent;font-weight:400;font-style:normal;font-variant:normal;text-decoration:none;vertical-align:baseline;white-space:pre;white-space:pre-wrap;">Some text</span></p><br /><p dir="ltr" style="line-height:1.38;margin-top:12pt;margin-bottom:12pt;"><span style="font-size:12pt;font-family:Arial;color:#000000;background-color:transparent;font-weight:400;font-style:normal;font-variant:normal;text-decoration:none;vertical-align:baseline;white-space:pre;white-space:pre-wrap;"><span style="border:none;display:inline-block;overflow:hidden;width:248px;height:248px;"><img src="data:image/png;base64," width="248" height="248" style="margin-left:0px;margin-top:0px;" /></span></span></p><br /></b>',
      )

      // image should be top-level and shouldn't have paragraph as a parent
      await expect(page.getByTestId('image-card-populated')).toBeVisible()
      await expect(page.locator('p:has([data-inkling-card="image"])')).toHaveCount(0)
    })
  })
  ...
  test.describe('caption', function () {
    ...
    test('can delete node and undo without losing caption', async function () {
      ...
      await page.keyboard.press('ArrowUp')
      await page.waitForSelector('[data-inkling-card="image"][data-inkling-card-selected="true"]')
      await page.keyboard.press('Backspace')
      await expect(page.locator('[data-inkling-card="image"]')).not.toBeVisible()
    ...
    test('can toggle between alt and caption', async function () {
      await expect(page.getByTestId('image-caption-editor')).toHaveText('This is a caption')
      await page.click('[data-inkling-card="image"]') // alt toggle not shown until selected
  ...
  test('can drag image card onto image card to create gallery', async function () {
    ...
    // Wait for both images to finish loading/rendering before dragging so the
    // drag preview and droppable hit-testing are stable on CI.
    ...
    const imageCard1BBox = await getBoundingBox(page.locator('[data-inkling-card="image"]').nth(0))
    const imageCard2BBox = await getBoundingBox(page.locator('[data-inkling-card="image"]').nth(1))
...
async function insertImage(page: Page, image = 'large-image.png') {
  const filePath = fixture(image)
...
async function insertEmptyImageCard(page: Page) {
  await loadSerializedState(page, {
    root: {
      children: [
        {
    ...
    results: [
      {
        ...
        tags: ['cat', 'funny', 'fall', 'submit', 'play'],
        flags: [],
        hasaudio: false,
      },
      {
        ...
        tags: ['Steve Harvey', 'oh', 'You Crazy', 'point', 'stop'],
        flags: [],
        hasaudio: false,
      },
      {
        ...
        tags: ['cats', 'animals', 'reaction', 'wow', 'surprised'],
        flags: [],
        hasaudio: false,
      },
    ],
...
test.describe('Image card - Klipy GIF provider', () => {
  let page: Page
  test.beforeAll(async ({ browser }) => {
    page = await browser.newPage()
  })
      ...
      html`
        <div data-lexical-decorator="true" contenteditable="false">
          <div data-inkling-card-editing="false" data-inkling-card-selected="true" data-inkling-card="image">
            <figure data-inkling-card-width="regular">
              <div>
                <img alt="" src="https://static.klipy.com/gif/klipy-cat.gif" />
              </div>
              <figcaption>
                <div data-testid="image-caption-editor" data-inkling-allow-clickthrough="true">
                  <div>
