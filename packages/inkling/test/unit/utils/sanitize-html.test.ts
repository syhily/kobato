...
describe('Utils: sanitize-html', () => {
  ...
  it.each(['ftp://example.com', 'javascript:alert(1)', 'mailto:hello@example.com', 'data:text/plain,hello'])(
    'disallows non-http protocols: %s',
    (href) => {
  ...
  it.each(['foo/bar', './foo', '../foo'])('disallows relative links: %s', (href) => {
    const sanitizedHtml = sanitizeHtml(`<a href="${href}">link</a>`)
    expect(sanitizedHtml).toEqual('<a>link</a>')
  })
