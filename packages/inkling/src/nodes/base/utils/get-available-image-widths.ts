export const getAvailableImageWidths = function (
  image: { width: number },
  ...
  if (
    image.width > availableImageWidths[availableImageWidths.length - 1] &&
    image.width < imageWidths[imageWidths.length - 1]
  ) {
