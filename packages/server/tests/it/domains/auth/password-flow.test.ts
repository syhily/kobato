...
import { issueResetToken, issueSetupToken, peekToken } from '@kobato/server/domains/auth/verification-tokens'
...
vi.mock('@kobato/server/domains/auth/passkey/service', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@kobato/server/domains/auth/passkey/service')>()
