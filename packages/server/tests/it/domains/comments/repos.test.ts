...
import {
  ...
  searchPages,
} from '@kobato/server/domains/comments/repos/admin-query'
import {
  insertComment,
...
import {
  ...
  pendingComments,
} from '@kobato/server/domains/comments/repos/public-query/digest'
import {
  countCommentsAndRoots,
  findChildComments,
  findCommentRootId,
  findRootComments,
} from '@kobato/server/domains/comments/repos/public-query/threads'
import {
  adminPendingWhere,
  ...
  whereTarget,
} from '@kobato/server/domains/comments/repos/shared'
...
} from '@kobato/server/domains/comments/services/lookup'
import { countMyComments, listMyCommentEntities } from '@kobato/server/domains/comments/services/mine-comments'
import {
...
describe('comments/repos/mutate — updateCommentBodyAndContent', () => {
  it('rewrites body + content', async () => {
    const id = await seedComment({ content: 'old' })
    await updateCommentBodyAndContent(db, id, EMPTY_LEXICAL_COMMENT_BODY, 'rewritten')
    const rows = await db.select({ content: comment.content }).from(comment).where(eq(comment.id, id))
...
describe('comments/repos/mutate — updateOwnCommentBody', () => {
  it('returns 0 when updated_at has drifted (optimistic lock)', async () => {
    const id = await seedComment({ content: 'old' })
    const n = await updateOwnCommentBody(db, id, EMPTY_LEXICAL_COMMENT_BODY, 'x', new Date('2000-01-01'))
    expect(n).toBe(0)
  })
  it('returns 1 when the optimistic lock matches', async () => {
    const rows = await db
      ...
      .returning()
    const id = rows[0]!.id
    const n = await updateOwnCommentBody(db, id, EMPTY_LEXICAL_COMMENT_BODY, 'y', new Date('2025-01-01'))
    expect(n).toBe(1)
  })
})
...
describe('comments/repos/mutate — updateOwnCommentBodyAndPending', () => {
  it('re-pends the comment when the lock matches', async () => {
    const rows = await db
      ...
      .returning()
    const id = rows[0]!.id
    const n = await updateOwnCommentBodyAndPending(db, id, EMPTY_LEXICAL_COMMENT_BODY, 'y', new Date('2025-01-01'))
    expect(n).toBe(1)
