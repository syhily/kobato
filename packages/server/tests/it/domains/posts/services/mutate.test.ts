...
import { content as contentTable, postSearchIndex } from '@kobato/server/infra/db/schema/content'
...
// runs for real.
vi.mock('@kobato/server/domains/posts/services/search-index', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@kobato/server/domains/posts/services/search-index')>()
  return {
...
vi.mock('@kobato/server/domains/content/invalidate', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@kobato/server/domains/content/invalidate')>()
...
const { createPost, deletePost, restorePost, unpublishPost, updatePostMeta } =
  await import('@kobato/server/domains/posts/services/mutate')
const { indexPost, removePostIndex } = await import('@kobato/server/domains/posts/services/search-index')
const { invalidateContent } = await import('@kobato/server/domains/content/invalidate')
