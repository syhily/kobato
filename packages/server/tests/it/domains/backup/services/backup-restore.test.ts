...
import { createBackup, getBackupBuffer, wireBackupSnapshots } from '@kobato/server/domains/backup/services/backup'
import { findBackupByTimestamp } from '@kobato/server/infra/db/operations/backup'
...
describe('backup and restore integration', () => {
  it('creates a two-file tar.gz archive (content + analytics) that round-trips through the storage backend', async () => {
    ...
    const { openDatabase, closeDatabase } = await import('@kobato/server/infra/db/database')
    ...
    const { openAnalyticsDatabase, closeAnalyticsDatabase } = await import('@kobato/server/infra/analytics/duckdb')
    const { ACCESS_LOG_DDL } = await import('@kobato/server/domains/analytics/services/access-log')
    ...
    const { stageBackup } = await import('@kobato/server/domains/backup/services/restore')
