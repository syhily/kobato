...
const realVerificationTokens = await vi.importActual<typeof import('@kobato/server/domains/auth/verification-tokens')>(
  '@kobato/server/domains/auth/verification-tokens',
)
const issueSetupToken = vi.fn(realVerificationTokens.issueSetupToken)

vi.mock('@kobato/server/domains/auth/verification-tokens', () => ({
...
const { inviteAuthorWithRollback } = await import('@kobato/server/domains/users/services/admin')
