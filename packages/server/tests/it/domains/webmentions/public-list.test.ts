...
import { ensureMetric } from '@kobato/server/infra/db/operations/metric'
import { setWebmentionStatus, upsertWebmention } from '@kobato/server/infra/db/operations/webmention'
...
// The public display surface (`content/v1/webmentions`, split-plan
...
describe('integration / content/v1/webmentions (headless display feed)', () => {
  ...
  it('404s on an unknown page_key (NOT_FOUND, same as comments)', async () => {
    await expect(
      call(contentPublicRouter.listWebmentions, { page_key: 'no-such-key' }, { context: makePublicCtx({ db }) }),
    ).rejects.toThrow()
  })
})
