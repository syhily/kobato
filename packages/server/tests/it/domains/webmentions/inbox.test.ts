...
import { processWebmentionInboxRow, runWebmentionInboxBatch } from '@kobato/server/domains/webmentions/inbox'
import {
  listWebmentionInbox,
...
vi.mock('@kobato/server/domains/webmentions/email', () => ({
  sendNewWebmention: vi.fn(async () => ({ ok: true })),
}))

const { sendNewWebmention } = await import('@kobato/server/domains/webmentions/email')
