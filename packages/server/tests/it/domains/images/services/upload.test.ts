...
import { buildObjectKey } from '@kobato/server/domains/images/key'
...
vi.mock('@kobato/server/infra/image/process', () => ({ processImageBuffer: processImageBufferMock }))
...
vi.mock('@kobato/server/infra/db/operations/image', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@kobato/server/infra/db/operations/image')>()
  return {
    ...actual,
...
const { assertImageUploadAllowed, uploadImage } = await import('@kobato/server/domains/images/services/upload')
const { upsertImageByStoragePath, insertImage } = await import('@kobato/server/infra/db/operations/image')
