...
//
//   1. `toCmsPage` / `toAdminRevisionDto` fall back to an empty body for
//      malformed stored payloads (defence-in-depth so a future direct
//      INSERT can't blank the public site).
//   2. The DTO field shape stays stable (id stringification, ISO dates).

const { toCmsPage } = await import('@kobato/server/domains/pages/projection')
const { toAdminRevisionDto } = await import('@kobato/server/domains/content/projection')
...
describe('cms/pages/projection — toCmsPage', () => {
  ...
  it('falls back to an empty body for malformed jsonb that bypassed the API perimeter', () => {
    const dto = toCmsPage(
      metaRow({ id: 1, publishedRevisionId: 200 }),
...
describe('content/projection — toAdminRevisionDto', () => {
  it('stringifies bigint ids and ISO-encodes timestamps', () => {
    const dto = toAdminRevisionDto(
      contentRow({
        ...
        revisionNo: 7,
        status: 'published',
        body: lexBody([lexParagraphNode('Hi')]),
        authorId: 99,
      }),
    )
  ...
  it('falls back to an empty body for malformed jsonb so the editor never hydrates with garbage', () => {
    const dto = toAdminRevisionDto(contentRow({ body: [{ _type: 'block' }] as unknown as ContentRow['body'] }))
    expect(dto.body).toEqual(EMPTY_LEXICAL_BODY)
  })
