import type { ContentEntityAdapter, ForceOverwriteEntry } from '@kobato/server/domains/content/lifecycle'
...
// image-sync.test.ts).
vi.mock('@kobato/server/domains/content/services/image-sync', { spy: true })
vi.mock('@kobato/server/domains/content/revisions', { spy: true })

const { loadDraftPreviewBySlug, previewBody, saveBody } = await import('@kobato/server/domains/content/lifecycle')
const { syncLibraryImageBlocks } = await import('@kobato/server/domains/content/services/image-sync')
const { findLatestRevision } = await import('@kobato/server/domains/content/revisions')
