...
vi.mock('@kobato/server/http/app', () => ({
  createApiApp: vi.fn(() => new Hono()),
}))

vi.mock('@kobato/server/http/resources/analytics', () => ({
  analyticsEventsRouter: new Hono(),
}))

vi.mock('@kobato/server/http/resources/assets', () => ({
  assetsRouter: new Hono(),
}))

vi.mock('@kobato/server/http/resources/backup', () => ({
  backupRouter: new Hono(),
}))

vi.mock('@kobato/server/http/resources/branding', () => ({
  brandingRouter: new Hono(),
}))

vi.mock('@kobato/server/http/resources/feed', () => ({
  feedRouter: new Hono(),
}))

vi.mock('@kobato/server/http/resources/fonts', () => ({
  fontsRouter: new Hono(),
}))

vi.mock('@kobato/server/http/resources/fonts-package', () => ({
  fontsPackageRouter: new Hono(),
}))

vi.mock('@kobato/server/http/resources/images', () => ({
  imagesRouter: new Hono(),
}))

vi.mock('@kobato/server/http/resources/maxmind', () => ({
  maxmindRouter: new Hono(),
}))

vi.mock('@kobato/server/http/resources/music-proxy', () => ({
  musicProxyRouter: new Hono(),
}))

vi.mock('@kobato/server/http/resources/redirects', () => ({
  redirectsRouter: new Hono(),
}))

vi.mock('@kobato/server/http/resources/sitemap', () => ({
...
vi.mock('@kobato/server/infra/logger/sanitizer', () => ({
  sanitizeReqHeaders: vi.fn((headers) => headers),
  resBindings: vi.fn(() => ({})),
}))

import { buildCspHeader, configureMiddleware, buildLoadContext } from '@kobato/server/http/middleware-pipeline'
