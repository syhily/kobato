...
import { saveBody, previewBody } from '@kobato/server/domains/content/lifecycle'
...
describe('adminPagesRouter.preview', () => {
  it('returns html and headings', async () => {
    ...
    const ctx = makeAuthedCtx({ db })
    const res = await call(adminPagesRouter.preview, { body: EMPTY_LEXICAL_BODY }, { context: ctx })
