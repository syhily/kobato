...
import { issueCommentToken } from '@kobato/server/domains/comments/services/token'
...
import { serializeCommentTokensCookie } from '@kobato/shared/utils/comment-token'
...
vi.mock('@kobato/server/domains/comments/services/public-query', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@kobato/server/domains/comments/services/public-query')>()
  return { ...actual, loadComments: vi.fn(actual.loadComments) }
})

const publicQuery = await import('@kobato/server/domains/comments/services/public-query')
const { avatarRouter } = await import('@kobato/server/http/controllers/avatar.controller')
const { commentsPublicRouter } = await import('@kobato/server/http/controllers/comments-public.controller')
const commentsRouter = commentsPublicRouter
const { likesRouter } = await import('@kobato/server/http/controllers/likes.controller')
...
const validBody = lexCommentBody('hello')
...
describe('commentsRouter.edit', () => {
  it('throws BAD_REQUEST when rid is not numeric', async () => {
    const ctx = makePublicCtx({ db })
    await expect(
      call(commentsRouter.edit, { rid: 'not-a-number', body: EMPTY_LEXICAL_COMMENT_BODY }, { context: ctx }),
  ...
  it('lets an admin edit any comment body', async () => {
    ...
    const newBody = lexCommentBody('moderated')
