import {
  ...
  loadCommentsSchema,
} from '@kobato/server/domains/comments/schema'
    ...
    children: [
      {
        ...
        textStyle: '',
        children: [
          { detail: 0, format: 0, mode: 'normal', style: '', text: 'Thoughtful comment.', type: 'text', version: 1 },
        ],
      },
    ],
  },
...
describe('commentReplySchema anti-spam', () => {
  ...
  it('rejects a body that violates the Lexical comment dialect', async () => {
    // Headings are rejected because the comment dialect only allows
    // paragraph/quote/list/code/mathBlock root blocks. Validation
    // surfaces the field path under `body.root`, so we just assert the
      ...
      commentEditSchema.parseAsync({
        ...
        body: {
          root: {
            ...
            type: 'root',
            children: [
              { direction: null, format: '', indent: 0, version: 1, type: 'heading', tag: 'h2', children: [] },
            ],
          },
        },
      }),
