...
vi.mock('@kobato/server/infra/email/admin-notification', () => ({
  sendAdminNotification: vi.fn(async () => ({ ok: true })),
}))

const { sendAdminNotification } = await import('@kobato/server/infra/email/admin-notification')
const { sendNewFriendApplication } = await import('@kobato/server/domains/friends/email')
