import type { EnrichedAccessEvent, RawAccessEvent } from '@kobato/server/domains/analytics/types'
...
vi.mock('@kobato/server/domains/analytics/enrich', () => enrichMock)
vi.mock('@kobato/server/domains/analytics/services/batcher', () => batcherMock)
vi.mock('@kobato/server/domains/analytics/services/pv-batcher', () => pvMock)
vi.mock('@kobato/shared/config/getters', () => ({
  getBlogSettingsBundleSync: () => ({
    analytics: { analytics: { trackAdmin: false, keepBotRows: settings.keepBotRows } },
  }),
}))

const { trackPageView } = await import('@kobato/server/domains/analytics/track')
