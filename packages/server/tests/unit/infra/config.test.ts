import {
  ...
  loadServerConfig,
  migrateLegacyKeys,
} from '@kobato/server/infra/config'
...
describe('infra/config — loadConfig', () => {
  ...
  it('applies the api section defaults (allowedOrigins/trustedProxy/readRateLimit)', () => {
    const dir = makeTmpDir()
    const path = configPathIn(dir)
    withConfigArg(path)
    const env = loadConfig()
    expect(env['api.allowedOrigins']).toEqual([])
    expect(env['api.trustedProxy']).toEqual([])
    expect(env['api.readRateLimitPerMinute']).toBe(300)
  })

  it('applies the public.frontendUrl default and reads it back from file/env', () => {
    const dir = makeTmpDir()
    const path = configPathIn(dir)
    withConfigArg(path)
    expect(loadConfig()['public.frontendUrl']).toBe('')
    writeConfig(path, { public: { frontendUrl: 'https://front.example.com' } })
    expect(loadConfig()['public.frontendUrl']).toBe('https://front.example.com')
    vi.stubEnv('public__frontendUrl', 'https://cdn-front.example.com')
    expect(loadConfig()['public.frontendUrl']).toBe('https://cdn-front.example.com')
  })

  it('reads api section values from the config file and env', () => {
    const dir = makeTmpDir()
    const path = configPathIn(dir)
    withConfigArg(path)
    writeConfig(path, {
      api: { allowedOrigins: ['https://front.example.com'], trustedProxy: ['10.0.0.0/8'], readRateLimitPerMinute: 120 },
    })
    const env = loadConfig()
    expect(env['api.allowedOrigins']).toEqual(['https://front.example.com'])
    expect(env['api.trustedProxy']).toEqual(['10.0.0.0/8'])
    expect(env['api.readRateLimitPerMinute']).toBe(120)
    vi.stubEnv('api__allowedOrigins', 'https://a.example.com,https://b.example.com')
    vi.stubEnv('api__readRateLimitPerMinute', '60')
    const overridden = loadConfig()
    // The env layer keeps raw strings; the file converges to the parsed
    // array after validation.
    expect(overridden['api.allowedOrigins']).toBe('https://a.example.com,https://b.example.com')
    expect(overridden['api.readRateLimitPerMinute']).toBe('60')
    const written = JSON.parse(readFileSync(path, 'utf8')) as {
      api?: { allowedOrigins?: string; readRateLimitPerMinute?: number }
    }
    // The file converges to the raw env form (the schema splits on the
    // next boot); the validation-time value is what the transform yields.
    expect(written.api?.allowedOrigins).toBe('https://a.example.com,https://b.example.com')
    expect(written.api?.readRateLimitPerMinute).toBe(60)
  })
