...
function importModule(key: string | undefined) {
  vi.doMock('@kobato/server/infra/config', () => ({
    serverConfig: {
      server: {},
      database: {},
  ...
  return import('@kobato/server/infra/crypto/secret-encryption')
}

function clean() {
  vi.doUnmock('@kobato/server/infra/config')
  vi.doUnmock('@kobato/server/infra/logger')
  vi.resetModules()
}
