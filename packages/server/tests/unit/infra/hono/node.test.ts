...
vi.mock('@kobato/server/infra/hono/helpers', async () => {
  const actual = await vi.importActual<typeof import('@kobato/server/infra/hono/helpers')>(
    '@kobato/server/infra/hono/helpers',
  )
  return {
    ...actual,
...
vi.mock('@kobato/server/infra/hono/dev-server-ref', () => ({
...
const { createHonoServer } = await import('@kobato/server/infra/hono/node')
