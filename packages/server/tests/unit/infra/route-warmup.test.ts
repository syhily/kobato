...
import type { RouteManifest } from '@kobato/shared/constants/route-warmup'

import { deriveTier2RouteIds, routeWarmupPlugin, type RouteWarmupOptions } from '@kobato/server/infra/route-warmup'

// Contract-test table mirroring the pre-split single-app configuration:
// every bucket fed by its prefix, so the tier/derivation assertions below
// exercise the full four-bucket shape.
const TEST_OPTIONS: RouteWarmupOptions = {
...
async function invokeWriteBundle(ctx: ReturnType<typeof buildPluginContext>['ctx']): Promise<void> {
  const plugin = routeWarmupPlugin(TEST_OPTIONS)
...
describe('route-warmup tier derivation', () => {
  ...
  it('buckets route IDs by prefix and skips unprefixed IDs', () => {
    ...
    expect(deriveTier2RouteIds(manifest, TEST_OPTIONS.tier1Routes, TEST_OPTIONS.tier2Prefixes)).toEqual({
  ...
  it('excludes TIER1 members from prefix buckets even when they carry a prefix', () => {
    ...
    for (const id of TEST_OPTIONS.tier1Routes) {
      const buckets = deriveTier2RouteIds(manifestOf([id]), TEST_OPTIONS.tier1Routes, TEST_OPTIONS.tier2Prefixes)
      expect(Object.values(buckets).flat(), `TIER1 route "${id}" leaked into a tier-2 bucket`).toEqual([])
    }
  })

  it('leaves buckets empty for prefixes the app table does not cover', () => {
    // An app with only public routes (apps/public) must produce empty
    // admin/editor/auth buckets — the runtime manifest shape still carries
    // all four keys, so the root layouts' `collectTier2Chunks` can read
    // them unconditionally.
    const manifest = manifestOf(['root', 'routes/public/home', 'routes/public/archives'])
    const buckets = deriveTier2RouteIds(
      manifest,
      ['root', 'routes/public/layout', 'routes/public/home'],
      [['routes/public/', 'public']],
    )
    expect(buckets.public).toEqual(['routes/public/archives'])
    expect(buckets.admin).toEqual([])
    expect(buckets.editor).toEqual([])
    expect(buckets.auth).toEqual([])
  })
})

describe('route-warmup tier derivation — per-app routes.ts coverage', () => {
  // The "every route lands in exactly one warmup bucket" contract moved
  // with the app split into the per-app suites:
  //   apps/core/tests/unit/infra/route-warmup-coverage.test.ts
  //   apps/public/tests/unit/infra/route-warmup-coverage.test.ts
  it('is covered by the per-app route-table tests', () => {
