...
function createFakeQuery<TRow>(rows: TRow[]) {
  ...
  return {
    query: query as unknown as Parameters<typeof import('@kobato/server/infra/db/operations/admin-list').applyPage>[0],
    calls,
  }
}
...
describe('infra/db/operations/admin-list — assembleWhere', () => {
  it('returns undefined for an empty conditions array', async () => {
    const { assembleWhere } = await import('@kobato/server/infra/db/operations/admin-list')
...
describe('infra/db/operations/admin-list — applyPage', () => {
  it('returns rows from the builder untouched when no page is given', async () => {
    const { applyPage } = await import('@kobato/server/infra/db/operations/admin-list')
...
describe('infra/db/operations/admin-list — withUploader', () => {
  async function createImageUploader() {
    const { withUploader } = await import('@kobato/server/infra/db/operations/admin-list')
    return withUploader({
      table: image,
      idColumn: image.id,
