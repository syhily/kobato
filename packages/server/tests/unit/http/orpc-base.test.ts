...
import {
  ...
  type FrontendAuth,
  type HandlerContext,
} from '@kobato/server/http/orpc-base'
...
describe('commentTokenCookie oRPC middleware', () => {
  ...
  it('merges the X-Kobato-Comment-Token jar behind a verified frontendAuth', async () => {
    // The proxy-chain jar merge (phase 0.6): `frontendAuth` is what
    // `frontendKeyAuth` injects after verifying the frontend JWT — the
    // unit test presets it directly (the real JWT → frontendAuth chain is
    // covered by the it-tier apikey tests).
    const headerJar = { 'pk-2': [{ token: 'tok-2', expiresAt: 999 }] }
    const ctx = makeCookieCtx() as HandlerContext & { frontendAuth: FrontendAuth | null }
    ctx.frontendAuth = { keyId: 'key-1', scopes: ['content:write'], forwardedAddress: null }
    ctx.request = new Request('http://localhost/rpc/ct/read', {
      headers: { 'X-Kobato-Comment-Token': encodeURIComponent(JSON.stringify(headerJar)) },
    })
    const res = await call(tokenRouter.read, {}, { context: ctx })
    // Cookie jar page + header jar page, overlaid.
    expect(res).toEqual({ pages: ['pk-1', 'pk-2'] })
  })

  it('ignores the X-Kobato-Comment-Token header without a verified frontendAuth', async () => {
    const headerJar = { 'pk-2': [{ token: 'tok-2', expiresAt: 999 }] }
    const ctx = makeCookieCtx()
    ctx.request = new Request('http://localhost/rpc/ct/read', {
      headers: { 'X-Kobato-Comment-Token': encodeURIComponent(JSON.stringify(headerJar)) },
    ...
    const res = await call(tokenRouter.read, {}, { context: ctx })
    // Anonymous requests cannot inject tokens via the header.
    expect(res).toEqual({ pages: ['pk-1'] })
  })
