...
describe('requestContextMiddleware', () => {
  ...
  async function buildApp(mocks: { sessionCtx?: { dirty?: boolean } } = {}) {
    ...
    vi.doMock('@kobato/server/bootstrap/db-lifecycle', () => ({
      getDb: vi.fn(() => fakeDb),
    }))
    vi.doMock('@kobato/server/domains/auth/primitives', () => ({
    ...
    vi.doMock('@kobato/server/domains/auth/csrf', () => ({
      ensureCsrfToken,
    ...
    vi.doMock('@kobato/server/domains/auth/session-storage', () => ({
      commitSessionWithMaxAge,
      SESSION_COOKIE_NAME: '__session',
    }))
    vi.doMock('@kobato/server/http/middlewares/visitor-cookie', () => ({
      isExempt: (path: string) => path.startsWith('/feed'),
    }))
    const getClientAddress = vi.fn().mockReturnValue('192.168.1.1')
    vi.doMock('@kobato/server/http/utils/client-address', () => ({
      getClientAddress,
    }))
