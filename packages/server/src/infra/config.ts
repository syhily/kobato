...
export const CONFIG_TABLE = [
  ...
  },
  {
    ...
    // Write-interaction CORS allowlist for the headless REST face
    // (phase 0.6). Anonymous reads answer `Access-Control-Allow-Origin: *`;
    // credentialed writes require the Origin to be listed here.
    schema: z
  ...
  {
    path: ['api', 'trustedProxy'],
    // Trusted front-end server addresses (IPs or CIDR-ish prefixes) whose
    // read traffic is exempt from the per-IP read limit — otherwise the
    // official frontend's SSR reads all count as one IP.
    schema: z
      .union([z.string(), z.array(z.string())])
      .optional()
      .transform((value) => {
  ...
  {
    path: ['api', 'readRateLimitPerMinute'],
    // Loose per-IP read limit for the public Content API; 0 disables.
    schema: z.coerce.number().int().min(0).max(10000).default(300),
    fileDefault: 300,
  },
  {
    path: ['public', 'frontendUrl'],
    // The official frontend's browser-reachable origin (plan 0.5 §5 /
    // 阶段 1 frontend 配置面). Empty = the frontend is not separately
    // deployed (single-origin or in-process forms) and the admin app
    // keeps same-origin public links. When set, the admin editor mints a
    // short-lived preview token and points its "view on the public site"
    // links at this origin with `?preview_token=…` (the cross-domain
    // draft-preview credential — see `domains/preview-token`).
...
export interface ServerConfig {
  ...
  api: {
    ...
    /** Trusted front-end server addresses exempt from the read limit. */
    trustedProxy: string[]
    /** Loose per-IP read limit for the public Content API; 0 disables. */
    readRateLimitPerMinute: number
  }
  public: {
    /** The official frontend's browser-reachable origin ('' = same-origin deployment). */
    frontendUrl: string
  }
}
