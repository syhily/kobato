...
import { sanitizeHtml } from '@kobato/server/render/sanitize-html'
import {
...
// (`@kobato/shared/lexical/html-manifest`) as an HTML string, so feeds
// and other string sinks can render without a DOM or a component tree.
// Lives in the server package (moved from the editor package in stage 4);
// the React twin stays in `@kobato/editor/lexical-html/LexicalBody`.
//
...
//   - `rss`     — classless degraded form for feed strings (math/code
//     fall back to escaped TeX/text, twoColumn concatenates, music keeps
//     the figure+audio form)
...
/** Feed-shaped music metadata (name/artist/audio/cover). */
...
function renderCode(node: Extract<LexicalNonContainerBlockNode, { type: 'code' }>, ctx: RenderContext): string {
  ...
  // the retired PT feed's RSS branch.
...
function renderImage(node: Extract<LexicalNonContainerBlockNode, { type: 'image' }>, ctx: RenderContext): string {
  if (ctx.mode === 'rss') {
    // Feed form: bare figure, alt/width/height only when set.
...
function renderMathMarkup(
  ...
  if (ctx.mode === 'rss') {
    // Feed readers cannot safely consume raw MathML/SVG — the RSS
    // branch always falls back to plain TeX inside `<code>`.
    return undefined
  }
...
function renderSolution(node: Extract<LexicalBlockNode, { type: 'solution' }>, ctx: RenderContext): string {
  if (ctx.mode === 'rss') {
    // The RSS branch renders the children bare — no wrapper.
...
function renderTwoColumn(node: Extract<LexicalBlockNode, { type: 'twoColumn' }>, ctx: RenderContext): string {
  ...
  if (ctx.mode === 'rss') {
    // The RSS branch concatenates the panes — no wrapper.
