...
import { ManagedEngine } from '@kobato/server/bootstrap/managed-engine'
import { rescheduleGeoipUpdate } from '@kobato/server/domains/analytics/geoip-scheduler'
import {
  rescheduleArchive,
  scheduleNextArchive,
  wireArchiveScheduler,
} from '@kobato/server/domains/audit/services/scheduler'
import { wireSessionStorageDb } from '@kobato/server/domains/auth/session-storage'
import { wireRestoreMachine } from '@kobato/server/domains/backup/restore-machine'
import { rescheduleBackup, wireBackupScheduler } from '@kobato/server/domains/backup/scheduler'
import { wireBackupSnapshots } from '@kobato/server/domains/backup/services/backup'
import {
  cleanupPreRestoreFiles,
  rollbackPreRestoreFiles,
  sweepStaleRestoreDirs,
} from '@kobato/server/domains/backup/services/restore'
import { resetLikeTokenSweep, startLikeTokenSweep } from '@kobato/server/domains/comments/services/likes'
import { wireScheduledPublishScheduler } from '@kobato/server/domains/content/scheduled-publish'
import { refreshBlogSettings } from '@kobato/server/domains/settings/services/hydrate'
import { registerSectionChangeHandler } from '@kobato/server/domains/settings/services/section-changes'
import { wireWebmentionPostPublishHook } from '@kobato/server/domains/webmentions/enqueue'
import { wireWebmentionInboxScheduler } from '@kobato/server/domains/webmentions/inbox-scheduler'
import { wireWebmentionOutboxScheduler } from '@kobato/server/domains/webmentions/outbox-scheduler'
import { wireWebmentionReverifyScheduler } from '@kobato/server/domains/webmentions/reverify-scheduler'
import { wireKvSweepScheduler } from '@kobato/server/infra/cache/kv-maintenance'
...
import {
  ...
  resetAllBatchers,
} from '@kobato/server/infra/db/batcher-registry'
import {
  closeDatabase,
  openDatabase,
  resolveDatabasePath,
  type Database,
  type DatabaseHandle,
} from '@kobato/server/infra/db/database'
import { wireDbMaintenanceScheduler } from '@kobato/server/infra/db/maintenance'
...
import '@kobato/server/domains/analytics/services/batcher'
import '@kobato/server/domains/analytics/services/pv-batcher'
import '@kobato/server/domains/audit/services/batcher'
import {
  closeHttpServer,
  restartServer,
  setRestartDb,
  ...
  setServerPhase,
} from '@kobato/server/infra/lifecycle'
import { root } from '@kobato/server/infra/logger'
