...
import type { authorProc } from '@kobato/server/http/orpc-base'

import { recordAuditEventFromContext } from '@kobato/server/domains/audit/services/record'
import { previewBody, saveBody } from '@kobato/server/domains/content/lifecycle'
...
import { previewBodyInput, previewOutputDto, saveBodyInput, saveResultOutput } from '@kobato/shared/contracts/revision'
  ...
  const preview = proc
    ...
    .handler(({ input, context }) => {
      return previewBody(input.body, (body) =>
        renderLexicalBodyToHtml(body, [], (playerIds) => getPublicMusicMetasByIds(context.db, playerIds)),
      )
    })
