...
import type { LexicalBody, LexicalImageNode } from '@kobato/shared/lexical/schema'

import { findImagesByIds, updateImageNote } from '@kobato/server/infra/db/operations/image'
import { getPublicBaseUrl } from '@kobato/server/infra/storage/public-url'
...
export async function syncLibraryImageBlocks(db: Database, body: LexicalBody): Promise<void> {
  const targets: LexicalImageNode[] = []
  ...
  const idTargets: { id: number; target: LexicalImageNode }[] = []
