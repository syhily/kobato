...
import type { ContentType, PublishLatestResult, SaveDraftResult } from '@kobato/server/domains/content/schemas/revision'
...
import type { PortableTextHeading } from '@kobato/shared/legacy-pt/schema'
import type { LexicalBody } from '@kobato/shared/lexical/schema'
import type { RoleOrNull } from '@kobato/shared/utils/roles'

import { toAdminRevisionDto } from '@kobato/server/domains/content/projection'
import { publishLatestRevision, saveDraftRevision } from '@kobato/server/domains/content/repos/mutate'
import { findContentById, findLatestDraft, findLatestRevision } from '@kobato/server/domains/content/revisions'
import { rescheduleScheduledPublish } from '@kobato/server/domains/content/scheduled-publish'
import { syncLibraryImageBlocks } from '@kobato/server/domains/content/services/image-sync'
import { canonicalizeLexicalBody } from '@kobato/server/domains/lexical/services/canonicalize'
import { getLogger, type Logger } from '@kobato/server/infra/logger'
import { deriveSlug } from '@kobato/server/infra/slug/derive'
import { collectHeadingsLexical, collectImageStoragePaths } from '@kobato/shared/lexical/walk'
...
export interface ContentEntityAdapter<TMeta, TPreview> {
  ...
  afterPublish(db: Database, meta: TMeta, body: LexicalBody, warnings: string[]): Promise<void>
}
...
export async function saveBody<TMeta, TPreview>(
  ...
  const body = await canonicalizeLexicalBody(input.body)
  ...
  const headings = collectHeadingsLexical(body, deriveSlug)
...
export async function previewBody(
  rawBody: unknown,
  render: (body: LexicalBody) => Promise<string>,
  ...
  const body = await canonicalizeLexicalBody(rawBody)
