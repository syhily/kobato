...
import type { EntityType } from '@kobato/server/infra/db/target'

import { listMyComments } from '@kobato/server/domains/comments/repos/admin-query'
import { findParentCommentsByIds } from '@kobato/server/domains/comments/repos/public-query/by-id'
import {
...
import { EMPTY_LEXICAL_COMMENT_BODY, type LexicalCommentBody } from '@kobato/shared/lexical/comment-schema'
...
export async function loadMineCommentsPage(
  ...
  const items: MineCommentItem[] = rows.map((c) => {
    ...
    return {
      id: String(c.id),
      body: c.body ?? EMPTY_LEXICAL_COMMENT_BODY,
