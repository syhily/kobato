...
import { parseStoredBody } from '@kobato/server/domains/content/projection-helpers'
import { warmContentRenderCaches } from '@kobato/server/domains/content/render-warmup'
import { findContentById } from '@kobato/server/domains/content/revisions'
import { isLive, isPromoted } from '@kobato/server/domains/content/schemas/live-gate'
import { toAdminPostDto, toCmsPost, type AdminPostDto } from '@kobato/server/domains/posts/projection'
import { runPostPublishHooks } from '@kobato/server/domains/posts/publish-hooks'
import {
  insertPostMeta,
  restorePostMeta,
  softDeletePostMeta,
  updatePostMetaById,
} from '@kobato/server/domains/posts/repos/write'
import { indexPost, removePostIndex } from '@kobato/server/domains/posts/services/search-index'
import { assertOwnPostOr404, type UpsertPostMetaInput } from '@kobato/server/domains/posts/services/shared'
import {
  findPostMetaById,
  findPostMetaBySlug,
  findPostMetaBySlugForUpdate,
  findPublicPostMetaBySlug,
} from '@kobato/server/domains/posts/services/single'
import { findCategoryById, findCategoryNamesByIds } from '@kobato/server/infra/db/operations/category'
import { findTagNamesByPostId, findTagNamesByPostIds, setPostTags } from '@kobato/server/infra/db/operations/post-tag'
import { findTagsByNames, seedTagsIfMissing } from '@kobato/server/infra/db/operations/tag'
  ...
  mutations: {
    ...
    async afterMutation(db, meta, event) {
      ...
      if (event === 'update' && isPromoted(meta)) {
        ...
        const body = parseStoredBody(revision.body)
        if (body === null) {
    ...
    async afterRestore(db, indexable) {
      ...
      const body = parseStoredBody(indexable.body)
      if (body === null) {
      ...
      try {
        await indexPost(db, indexable.id, indexable.title, indexable.summary, body)
      } catch (err: unknown) {
