...
`packages/server/src/` is SSR-only: may import `@kobato/shared` and other `@kobato/server` modules, never `@kobato/client` or `@kobato/ui`. Four layers, strict one-way import graph:
...
- In-memory: `BlogSettingsBundle` (`@kobato/shared/config/types`). SSR reads slices via `requireBlogSettingsSection('<key>')`; UI uses per-section hooks. New UI MUST NOT read the whole bundle.
...
- Canonical: `@kobato/server/infra/slug/derive::deriveSlug(text)` — pipeline `pinyin-pro` → whitespace-collapse → `github-slugger`. Server-only (`pinyin-pro` must not reach the client). The fused explicit-or-derived resolver is `@kobato/server/infra/slug/resolve::resolveSlug`; the route-prefix fence + in-transaction reservation live in `@kobato/server/infra/slug/reservation`.
- Heading anchors: SSR loaders pre-compute via `collectHeadings(body, deriveSlug)`.
- Page ↔ post slugs share one namespace. Cross-table uniqueness is enforced by the `slug_registry` table: `@kobato/server/infra/slug/reservation::reserveSlugInTransaction` checks it in-transaction, and `@kobato/server/domains/content/slug-conflict::rethrowSlugConflict` maps raced DB `UNIQUE` violations (`{post,page}.slug`, `slug_registry.slug`) to a clean 409.
...
- **How**: `recordAuditEventFromContext` from `@kobato/server/domains/audit/services/record`, called after the mutation succeeds.
...
- No barrel `index.ts` files anywhere inside `packages/server/src/`.
- No `*.server.ts` suffix inside `packages/server/src/` — it's redundant.
