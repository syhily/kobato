...
export function parseCommentTokensCookie(cookieHeader: string | null): CommentTokenCookie {
  ...
  return parseCommentTokenHeader(match.slice(`${COMMENT_TOKEN_COOKIE_NAME}=`.length))
}

/**
 * Parse a URL-encoded comment-token jar (the `X-Kobato-Comment-Token`
 * header form of the proxy chain, phase 0.6). The wire format mirrors the
 * `__comment_tokens` cookie value — the SDK's `token.ts` re-implements it
 * standalone (the published package cannot import workspace internals),
 * and the contract tests pin the two sides to the same shape.
 */
export function parseCommentTokenHeader(headerValue: string | null): CommentTokenCookie {
  if (!headerValue) {
    return {}
  }
  try {
    const raw = decodeURIComponent(headerValue)
