...
import type { AdminCommentsData, StatusCounts } from '@kobato/ui/admin/comments/useCommentsController'
...
// Minimal Lexical comment body — type-safe without constructing every node field.
const mockBody = lexCommentBody('hello')
...
describe('updateCommentBodyInPages', () => {
  it('updates the body of the matching comment', () => {
    const newBody = lexCommentBody('updated')
    const comments = [mockComment({ id: '1' }), mockComment({ id: '2' })]
