...
import { MusicPickerDialog } from '@kobato/ui/admin/editor-pickers/MusicPickerDialog'
import { CreateModeBanner } from '@kobato/ui/admin/editor-shared/CreateModeBanner'
import { TitleSlugStrip } from '@kobato/ui/admin/editor-shared/TitleSlugStrip'
import { ActionBanner } from '@kobato/ui/admin/editor-shell/ActionBanner'
import { DraftConflictDialog } from '@kobato/ui/admin/editor-shell/DraftConflictDialog'
import { EditorMetaPanel } from '@kobato/ui/admin/editor-shell/EditorMetaPanel'
import { EditorToolbar } from '@kobato/ui/admin/editor-shell/EditorToolbar'
import { FloatingPublishButton } from '@kobato/ui/admin/editor-shell/FloatingPublishButton'
import { PreviewPane } from '@kobato/ui/admin/editor-shell/PreviewPanel'
...
> {
  ...
  /** Public detail URL of the entity being edited (absolute + preview token when split). */
  publicPath: (slug: string) => string
  /** Absolute public-link origin ('' when the frontend is same-origin). */
  publicLinkOrigin: string
  /** Bare `key=value` preview-token query ('') when the deployment is same-origin. */
  previewQuery: string
    ...
    >
      ...
      {isEditing && state.previewBanner !== null ? (
        <ActionBanner
          ...
          linkPrefix={`${adapter.publicLinkOrigin}${adapter.bannerBasePath}`}
          previewQuery={adapter.previewQuery}
      ...
      >
        <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-2">
          ...
          <LexicalBodyEditor
            ...
            pickerRenderers={{
              renderImagePicker: (props) => <ImageLibraryPicker {...props} />,
              renderMusicPicker: (props) => <MusicPickerDialog {...props} />,
            }}
            floatingActions={
              isEditing ? (
