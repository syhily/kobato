...
import { useCreateDraft } from '@kobato/client/hooks/use-create-draft'
import { useLocalDraft } from '@kobato/client/hooks/use-local-draft'
...
import { useEditorKeyboardShortcuts } from '@kobato/ui/admin/editor-shell/use-editor-keyboard-shortcuts'
import { useEditorShellLayout } from '@kobato/ui/admin/editor-shell/use-editor-shell-layout'
...
// fresh object — a fresh object triggers infinite setState-during-render.
const EMPTY_BODY: LexicalBody = EMPTY_LEXICAL_BODY
...
>(args: UseEditorShellStateArgs<TMeta, TEntity, TUpsertMetaInput>): UseEditorShellStateOutput<TMeta> {
  ...
  const initialBody = useMemo<LexicalBody>(() => {
    return deriveBaselineRevision(detail)?.body ?? EMPTY_BODY
  }, [detail])

  const [body, setBody] = useState<LexicalBody>(initialBody)
  ...
  const replaceBody = useCallback((newBody: LexicalBody, key: string) => {
    setBody(newBody)
    setBodyKey(key)
  }, [])
  ...
  if (
    ...
    if (
      ...
      !areLexicalBodiesEquivalent(loadedLocalDraft.body, initialBody)
    ) {
  ...
  const adoptRevisionFromHistory = useCallback(
    (revision: { body: LexicalBody; revisionNo: number }) => {
      if (!isEditing || !detail) {
        return
      }
  ...
  const isBodyDirty = !areLexicalBodiesEquivalent(body, lastSavedBody)
