...
export interface EditorSidebarState<TMeta> {
  ...
  body: LexicalBody
  adoptRevisionFromHistory: (revision: { body: LexicalBody; revisionNo: number }) => void
}

/** Narrow slice consumed by the draft-conflict dialog. */
export interface EditorDialogState {
  conflict: { localBody: LexicalBody; localSavedAt: number } | null
...
export interface UseEditorShellStateOutput<TMeta> {
  ...
  body: LexicalBody
  setBody: React.Dispatch<React.SetStateAction<LexicalBody>>
  bodyKey: string
  initialBody: LexicalBody
  isEditing: boolean
  previewOpen: boolean
  setPreviewOpen: (updater: boolean | ((prev: boolean) => boolean)) => void
  metaOpen: boolean
