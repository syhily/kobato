...
- `db/` — `database.ts` (node:sqlite `DatabaseSync` open/close + pragma block), `migrate.ts`, drizzle schema, `operations/<entity>.ts` raw helpers; `insert-batcher` + `batcher-registry` for process-level write batchers; `maintenance.ts` daily vacuum/optimize job; `query-diagnostics.ts` 'sqlite.db.query' channel → debug-level statement timings (bound values redacted; vitest no-op).
- `job-registry.ts` — the process-level background jobs (backup, audit archive, scheduled publish, webmention queues, maintenance sweeps, GeoIP) each self-register `(name, nextDelayMs, run)` at import time; the hydration phase arms them all with one `startAllRegisteredJobs()` call after settings load. The registry also owns the shared lazy db-handle getter (`setJobHandleGetter` + `jobDb()`), so adding a timer means editing only its own module — no wire preamble, no server.ts edit. Timer mechanics stay in `scheduler-utils::scheduleJob`; enqueue-path nudges use `nudgeRegisteredJob(name)` (no-op until started). Registrations may carry a catalog `task: { key, recordHistory? }`: `scheduleJob` then tracks live state per task (`getSchedulerTaskState` — suspended / nextRunAt / running) and, with `recordHistory`, persists each fire to `job_run` via the recorder — the instrumentation backing `/admin/tasks`.
- `analytics/` — the DuckDB sidecar wrapper (`duckdb.ts`: open/close/path — zero business knowledge; the access_events DDL + appender live in `domains/analytics/services/access-log`).
...
- **Analytics sidecar** — DuckDB via `@duckdb/node-api`, one file at `storage.analyticsDatabase` (default `<storage.data>/analytics.duckdb`). Holds `access_events` only — a generic-column event table in the Cloudflare-Analytics-Engine style (ported from Slite): `index1` entity key (`post:<id>`/`page:<id>`/`''`), 16 opaque `blobN` columns whose semantics live in the `blobsMap` in `domains/analytics/services/access-log` (pinned by a contract test; `blob5` is the daily-salted visitor hash — raw IPs are never persisted; the 16-slot budget deliberately drops `osVersion`/`browserVersion` in favor of `browserType` — Slite never had version dims — and `blob16` is the reserved slot, always written `''`), `double1/2` lat/lng, plus one named `is_bot` boolean. Queries are built with Kysely as a compile-only SQL builder (`services/analytics-sql.ts`: DummyDriver, identifier whitelist, bound parameters only) and executed on a dedicated MVCC reader connection; the batcher writes through the Appender protocol. Recreated empty when missing, but INCLUDED in backups (the `.tar.gz` archive alongside the content DB and the env-converged `kobato.config.json`). A boot-time one-shot migration (`services/blob-migration.ts`, flag `system.analytics-blob-migration`) converts the legacy named-column `access_log` table — including sidecars restored from old backups — then drops it; the legacy table is probed BEFORE the flag so an analytics-only restore into an install whose flag row already exists still migrates.
- **Backup & restore** (`domains/backup/`) — the archive may be password-encrypted (chunked AES-256-GCM, `services/crypto.ts`: per-record seq‖flags AAD + FINAL marker so reordering/truncation fails authentication, `.enc` storage suffix); `BackupDecryptionError.kind` splits retryable `'auth'` failures from fatal `'format'` ones. The password resolver enters through `wireBackupEncryption` (Platform domains must not import settings), read via `resolveStoredEncryptionPassword()`; passwords are trimmed on both create and restore. Restore applies content DB → sidecar → config file IN ORDER, the config leg overwriting the live `kobato.config.json` for the next process restart. Boot runs `recoverPreRestoreFiles()` BEFORE the engines open (mid-swap crash → the `.pre-restore` sibling is renamed back, never deleted out from under a missing target); every restore path passes `onFailureFn` so pre-swap failures sweep the staged dir. Wrong passwords are `BackupPasswordError` (400); the stored-backup controller turns `EncryptedBackupPasswordRequired` into `{passwordRequired: true}`. Encrypted uploads park in `pending-uploads.ts` (10-minute token, max 4, LRU never evicts in-flight, all-busy park → 429) for the two-step `upload-restore` → `upload-restore/decrypt` flow (rate-limited, in-flight mutex → 409, dead token / structural failure → 410); restores audit `backup_restored`, downloads audit `backup_downloaded`. The 30s global request timeout exempts the restore endpoints (30 min). Pre-claim decrypt/extract progress merges with the machine phases in `restore-progress.ts`.
...
- **Daily maintenance** (04:30 site timezone): SQLite `incremental_vacuum` + `optimize` with page/freelist logging plus `job_run` history pruning (30 days / newest 200 per task, `infra/db/job-run-recorder::pruneJobRuns`); DuckDB 180-day retention DELETE + `CHECKPOINT` with row/file-size logging (`bootstrap/analytics-lifecycle.ts`); verification-token purge of rows expired >1 day (`domains/auth/token-purge-scheduler.ts`). Pure `ANALYZE` additionally runs after every bulk load (install seed, backup restore). Both DuckDB mutation windows — this retention job and the backup's CHECKPOINT + file copy (`snapshotAnalyticsTo`) — serialize on one module-level mutation lock, so a backup can never copy a file the retention job is mid-DELETE on.
- **`job_run` history** — opt-in execution history for background jobs (trigger, started/finished, duration, status, error), written by `infra/db/job-run-recorder` (wired in `db-lifecycle::wireDatabase`; unwired = silent no-op, writes never throw). Rows left `running` by a crash are swept to `failed` once at boot (`sweepOrphanedJobRuns`, called by `initDatabase` only — the restore rewiring skips it so in-flight jobs of the live process are never mislabeled).
...
- **Body projections (R9b, inkling migration)**: the save pipeline (`domains/content/lifecycle.ts`) computes the revision row's `bodyHtml` / `bodyText` / `bodyHtmlFeed` columns via `infra/pt/lexical-projection.ts` (the `@/inkling/headless` surface, jsdom-backed) over the canonical Lexical state; the feed variant's downgrade (math→TeX, code→plain pre, host-card substitution) is a state-copy transform in `shared/lexical/projection-state.ts`. Projection failure is best-effort (warn + NULL columns), and the no-op save short-circuit skips the computation — the repo/lifecycle share `repos/mutate::isEquivalentToPublishedLatest`. The dashboard「重建文章缓存」button (`admin.renders.reprojectBodies` → `domains/content/services/reproject-body-html.ts::reprojectBodyHtmlBatch`) re-derives the three columns corpus-wide in batches — the repair path when a renderer-side change leaves the saved projections stale; only drifted rows are written, never `updatedAt` / `clientRevisionToken`.
...
- **PT→Lexical backfill (R15)**: legacy PortableText `content`/`comment` rows convert row-by-row through the bespoke direct mapper `domains/pt/services/pt-to-lexical.ts` (every PT construct maps explicitly; unmapped constructs fail the row), gated per row by the zero-loss cross-check `domains/pt/services/pt-lexical-crosscheck.ts` (E1–E4; the E5 HTML round-trip is warnings-only). The executor `domains/content/services/pt-lexical-backfill.ts` (`runPtLexicalBackfill`, dry-run default) re-derives the projection columns, re-runs the R9a music meta snapshot, rebuilds the search index, and writes the `system.pt-lexical-backfill` flag row only on a zero-failure apply; `runPtLexicalBackfillAtBoot` applies it once per database at boot (BOOT_MODE 'apply' since R15b), serialized after the asset-URL backfill. The comment hasher and search reindex are injected collaborators (content must not import comments/posts — DAG).
