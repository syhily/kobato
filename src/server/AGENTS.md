...
- **Analytics sidecar** — DuckDB via `@duckdb/node-api`, one file at `storage.analyticsDatabase` (default `<storage.data>/analytics.duckdb`). Holds `access_log` only (append-heavy telemetry + dashboard scans); recreated empty when missing, but INCLUDED in backups (the `.tar.gz` archive alongside the content DB and the env-converged `kobato.config.json`). The batcher writes through the Appender protocol; queries run on a dedicated MVCC reader connection.
