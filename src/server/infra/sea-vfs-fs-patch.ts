...
// (`"useVfs": true`, Node 26.9.0 — still unfixed on nodejs/node main as of
// 2026-09-21): the mounted-VFS handler table (`lib/internal/vfs/setup.js`)
// implements `readvSync`/`writevSync` but NOT the async `readv`/`writev`,
// while the callback `fs.readv`/`fs.writev` (`lib/fs.js`) call
// `h.readv(fd, …)` / `h.writev(fd, …)` unguarded whenever any VFS layer is
// mounted. The result: ANY async `fs.writev` — most importantly
// `WriteStream._writev`, which fires whenever two chunks queue on a
// `fs.createWriteStream` — kills the process with
...
export function installSeaVfsFsPatch(): void {
