...
import type { BackupRow, NewBackup } from '@/server/infra/db/types'
...
export async function insertBackup(db: NodePgDatabase, values: NewBackup): Promise<BackupRow> {
...
export async function insertBackupIfMissing(db: NodePgDatabase, values: NewBackup): Promise<void> {
  await db.insert(backup).values(values).onConflictDoNothing({ target: backup.storagePath })
}

export async function findBackupByTimestamp(db: NodePgDatabase, timestamp: string): Promise<BackupRow | null> {
  const rows = await db.select().from(backup).where(eq(backup.timestamp, timestamp)).limit(1)
  return rows[0] ?? null
}

export async function listBackupRows(db: NodePgDatabase, limit?: number, offset = 0): Promise<BackupRow[]> {
...
export async function deleteBackupRow(db: NodePgDatabase, id: bigint): Promise<void> {
  await db.delete(backup).where(eq(backup.id, id))
}

/** Rows older than `cutoff` — used by the retention sweeper. */
export async function findOldBackupRows(db: NodePgDatabase, cutoff: Date): Promise<BackupRow[]> {
...
export async function listBackupStoragePaths(db: NodePgDatabase): Promise<string[]> {
...
export async function countLocalBackups(db: NodePgDatabase): Promise<number> {
