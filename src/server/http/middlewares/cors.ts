...
// from `/admin/settings/cors` without a restart.
//
//   - `cors.enabled === false` (default) → middleware is a no-op; no
//     CORS headers are added. Same-origin requests work as usual.
//   - `cors.enabled === true` + non-empty `origins` → only listed
//     origins receive CORS headers.
//   - `cors.enabled === true` + empty `origins` → mirror mode: the
...
export function corsMiddleware(): MiddlewareHandler<Env> {
  const handler = cors({
    origin: (origin) => {
      const settings = getBlogSettingsBundleSync()?.cors
      if (!settings || settings.cors.origins.length === 0) {
        return origin ?? ''
      }
      return settings.cors.origins.includes(origin ?? '') ? (origin ?? '') : ''
    },
