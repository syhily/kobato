 ...
 *
 * SQL callers must not bind the meta columns by hand: the repo-side
 * adapter `promotedPostWhere` (`posts/repos/shared.ts`) is the only
 ...
 * (see the warning on `isPromoted`). Call this through the repo-side
 * adapter `promotedPostWhere`, never with hand-bound columns.
 */
