...
export function parseButtonNode(ButtonNode: new (data: Record<string, unknown>) => LexicalNode) {
  return {
    div: (nodeElem: HTMLElement) => {
      const isButtonCard = nodeElem.classList?.contains('inkling-button-card')
      if (nodeElem.tagName === 'DIV' && isButtonCard) {
        return {
          conversion(domNode: HTMLElement) {
            const alignmentClass = nodeElem.className.match(/inkling-align-(left|center)/)
            ...
            const buttonNode = domNode?.querySelector('.inkling-btn')
