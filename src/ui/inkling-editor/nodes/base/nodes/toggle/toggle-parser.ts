...
export function parseToggleNode(ToggleNode: new (data: Record<string, unknown>) => LexicalNode) {
  return {
    div: (nodeElem: HTMLElement) => {
      const isKgToggleCard = nodeElem.classList?.contains('inkling-toggle-card')
      if (nodeElem.tagName === 'DIV' && isKgToggleCard) {
        return {
          conversion(domNode: HTMLElement) {
            const headingNode = domNode.querySelector('.inkling-toggle-heading-text')
            const heading = headingNode?.textContent ?? ''

            const contentNode = domNode.querySelector('.inkling-toggle-content')
