...
import { type IUnsplashProvider } from '@/ui/inkling-editor/unsplash/api/IUnsplashProvider'
import { fixturePhotos } from '@/ui/inkling-editor/unsplash/api/unsplashFixtures'
...
export class InMemoryUnsplashProvider implements IUnsplashProvider {
  ...
  public async searchPhotos(term: string): Promise<Photo[]> {
    ...
    const filteredPhotos = this.photos.filter(
      (photo) =>
        (photo.description && photo.description.toLowerCase().includes(term.toLowerCase())) ||
