...
const Context = React.createContext<TKContextValue>({
  tkNodeMap: {},
  tkCount: 0,
  addEditorTkNode: () => {},
  removeEditorTkNode: () => {},
...
export const TKContext = ({ children }: { children: React.ReactNode }) => {
  ...
  const editorTkNodeMapRef = React.useRef(new Map<string, Map<string, { topLevelNodeKey: string }>>())
  ...
  const addEditorTkNode = React.useCallback(
    (editorKey: string, topLevelNodeKey: string, tkNodeKey: string) => {
      const editorTkNodeMap = editorTkNodeMapRef.current
  ...
  const removeEditorTkNode = React.useCallback(
    (editorKey: string, tkNodeKey: string) => {
      const editorTkNodeMap = editorTkNodeMapRef.current
  ...
  const removeEditor = React.useCallback(
    (editorKey: string) => {
  ...
  const contextValue = React.useMemo<TKContextValue>(() => {
    return {
      tkNodeMap,
      tkCount,
      addEditorTkNode,
  ...
  }, [tkNodeMap, tkCount, addEditorTkNode, removeEditorTkNode, removeEditor])
