...
export function parseCommentFiltersFromSearchParams(searchParams: URLSearchParams): ActiveFilter[] {
  ...
  if (date && isDateFilterOperator(dateOp)) {
    const value = JSON.stringify({ date, op: dateOp })
    initialFilters.push({ field: 'date', value, label: dateFilterLabel({ date, op: dateOp }) })
  } else if (date) {
    ...
    // `DEFAULT_DATE_OPERATOR` matches Ghost's "on or before" default.
    const op = isDateFilterOperator(dateOp) ? dateOp : DEFAULT_DATE_OPERATOR
    const value = JSON.stringify({ date, op })
    initialFilters.push({ field: 'date', value, label: dateFilterLabel({ date, op }) })
  }
