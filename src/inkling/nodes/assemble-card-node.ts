 ...
 * (`@/inkling/nodes/cards/host-cards`) satisfy this shape too, so `defineCard`
 ...
 * delegated through the injection port (`@/inkling/nodes/card-decorate-slot`) the
 * wrapper layer (`@/inkling/nodes/decorate-card`) registers into — the inversion
 ...
 * wrapper-layer projection (`@/inkling/nodes/cards/card-wrappers`), the shim
 * modules (`@/inkling/nodes/AudioNode` and friends), and `defineCard`
 * (`@/inkling/nodes/cards/host-cards`) for host cards — assembles a card's
