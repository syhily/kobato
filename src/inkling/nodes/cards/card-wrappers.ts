 ...
 * modules (`@/inkling/nodes/AudioNode` and friends) re-export for their `$create*`
 ...
 * declaration modules so they stay React-free — `@/inkling/nodes/base` derives its
...
export const CARD_WRAPPER_NODES = CARD_DECLARATIONS.map((declaration) =>
  ...
  Object.assign({}, declaration, { node: assembleCardNodeOnce<LexicalNode>(declaration) }),
)
