import type { CardMenuSource } from '@/inkling/nodes/cards/card-menu-build'

import TableCardIcon from '@/inkling/assets/icons/inkling-card-type-table.svg?react'
import { INSERT_TABLE_COMMAND } from '@/inkling/plugins/behaviour/table'

/**
 * The table slash/plus menu entry as a pseudo `CardMenuSource` (the snippet
 * precedent in `@/inkling/nodes/cards/card-menu-build`): the table family is not a card, so
