...
import { $insertHorizontalRuleForUpdateScanTrigger, DIVIDER_REGEXP } from '@/inkling/markdown/card-shortcuts'
import { $createHorizontalRuleNode, INSERT_HORIZONTAL_RULE_COMMAND } from '@/inkling/nodes/HorizontalRuleNode'
import { registerUpdateScan } from '@/inkling/plugins/behaviour/update-scan'
import { getSelectedNode } from '@/inkling/utils/getSelectedNode'
...
export function resolveDividerScanTarget(editor: LexicalEditor): ParagraphNode | null {
  ...
  if (!node || !$isParagraphNode(node) || !DIVIDER_REGEXP.test(node.getTextContent())) {
    return null
  }
