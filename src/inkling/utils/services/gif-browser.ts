import {
  ...
  isGifResponse,
} from '@/inkling/utils/services/gif'
import { createRequestTrack, type RequestScheduler } from '@/inkling/utils/services/request-track'
import { runTrackedRequest, type ServiceMachine } from '@/inkling/utils/services/service-machine'
...
export function reduceGifKey(state: GifNavState, event: GifKeyEventData, geometry: GifGeometry): GifKeyTransition {
  ...
  const walkColumn = (step: 1 | -1): GifData | undefined => {
    if (highlighted?.columnIndex === undefined || highlighted.columnRowIndex === undefined) {
      return undefined
    }
  ...
  const moveRight = (): GifKeyTransition => {
    if (highlighted?.columnIndex === undefined) {
      return stay([PREVENT_DEFAULT])
    }
