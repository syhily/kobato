...
// `@/inkling/markdown/paste-dialect`, the card-aware round-trip dialect in
// `@/inkling/markdown/round-trip`), with each engine projecting its half:
//
// - **Inline delimiters** (`==mark==`, `~sub~`, `^sup^`): declared once in
//   `INLINE_DELIMITERS`. The `@lexical/markdown` engine projects the table
//   into its text-format transformers (`@/inkling/markdown/transformers-core`;
...
//
// The fence trigger BODIES stay in `@/inkling/markdown/card-shortcuts` (the seam
 ...
 * on this trigger (see `@/inkling/markdown/card-shortcuts`'s module comment). */
