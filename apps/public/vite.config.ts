...
const pkg = pkgSchema.parse(
  JSON.parse(readFileSync(resolve(import.meta.dirname, '..', 'core', 'package.json'), 'utf-8')),
)
  ...
  environments: {
    ssr: {
      ...
      ...(command === 'build'
        ? {
            build: {
              // Emit assets referenced by the server graph into
              // build/server/assets — Vite 8 defaults this off for
              // non-client consumers, and React Router's
              // `build.ssrEmitAssets` is no longer honored per-environment.
              emitAssets: true,
              rolldownOptions: {
                // The SEA frontend line injects this server entry into the
                // binary (`apps/public/build/server/index.js` — see
                // scripts/sea/server-entry-frontend.ts).
                input: 'src/server.ts',
              },
            },
          }
