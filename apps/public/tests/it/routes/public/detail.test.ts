...
import { lexBody } from '#/_helpers/lexical-body'
...
import { friend as friendTable } from '@kobato/server/infra/db/schema/friend'
...
import { weakEtag } from '@kobato/shared/http/etag'
...
// seam is the presentational LexicalBody renderer and a wrapped (not
// replaced) `resolveSessionContext` so the "no session re-resolution
// fallback" test can assert the loaders never consult it.
...
vi.mock('@kobato/server/domains/auth/primitives', async () => {
  const actual = await vi.importActual<typeof import('@kobato/server/domains/auth/primitives')>(
    '@kobato/server/domains/auth/primitives',
  )
...
vi.mock('@kobato/server/domains/posts/services/single', async () => {
  const actual = await vi.importActual<typeof import('@kobato/server/domains/posts/services/single')>(
    '@kobato/server/domains/posts/services/single',
  )
...
vi.mock('@kobato/server/domains/pages/services/public-query', async () => {
  const actual = await vi.importActual<typeof import('@kobato/server/domains/pages/services/public-query')>(
    '@kobato/server/domains/pages/services/public-query',
  )
...
// the page hides the section, and gate it to prove the music-meta
// resolution no longer queues behind it.
vi.mock('@kobato/server/domains/friends/service', async () => {
  const actual = await vi.importActual<typeof import('@kobato/server/domains/friends/service')>(
    '@kobato/server/domains/friends/service',
  )
...
vi.mock('@kobato/server/domains/taxonomies/tags/service', async () => {
  const actual = await vi.importActual<typeof import('@kobato/server/domains/taxonomies/tags/service')>(
    '@kobato/server/domains/taxonomies/tags/service',
  )
  mocks.getTagsByNames.mockImplementation(actual.getTagsByNames)
  return { ...actual, getTagsByNames: mocks.getTagsByNames }
})

vi.mock('@kobato/server/http/loaders/detail', async () => {
  const actual = await vi.importActual<typeof import('@kobato/server/http/loaders/detail')>(
    '@kobato/server/http/loaders/detail',
  )
...
// Wrapped: the music-meta tests gate one read of the page detail block
// and assert the detail orchestrator starts it without waiting for the
// friends read (the Lexical successor of the PT music prerender seam).
vi.mock('@kobato/server/domains/music/services/read', async () => {
  const actual = await vi.importActual<typeof import('@kobato/server/domains/music/services/read')>(
    '@kobato/server/domains/music/services/read',
  )
  mocks.getPublicMusicMetasByIds.mockImplementation(actual.getPublicMusicMetasByIds)
  return { ...actual, getPublicMusicMetasByIds: mocks.getPublicMusicMetasByIds }
})
...
async function seedPost(opts: Partial<typeof postTable.$inferInsert> & { body?: LexicalBody } = {}): Promise<number> {
  ...
  const revisions = await db
    .insert(contentTable)
    .values({ type: 'post', ownerId: postId, revisionNo: 1, status: 'published', body: body ?? EMPTY_LEXICAL_BODY })
    .returning({ id: contentTable.id })
  await db.update(postTable).set({ publishedRevisionId: revisions[0]!.id }).where(eq(postTable.id, postId))
  return postId
}

async function seedPage(opts: Partial<typeof pageTable.$inferInsert> & { body?: LexicalBody } = {}): Promise<number> {
  const { body, ...meta } = opts
  const rows = await db
    .insert(pageTable)
    .values({
  ...
  const revisions = await db
    .insert(contentTable)
    .values({ type: 'page', ownerId: pageId, revisionNo: 1, status: 'published', body: body ?? EMPTY_LEXICAL_BODY })
    .returning({ id: contentTable.id })
...
describe('routes/post.detail loader', () => {
  ...
  it('returns the canonical post payload for a real slug', async () => {
    ...
    expect(data.body).toEqual(EMPTY_LEXICAL_BODY)
  })
})
...
describe('routes/post.detail loader — ETag', () => {
  it('answers 304 when If-None-Match matches, with an unchanged etag', async () => {
    ...
    // The post DTO's `updated` projects `published_at`, so the etag keeps
    // the exact single-package inputs (`['post', id, publishedAt]`).
    ...
    // Headless form: the slim DB probe is gone (no database on the
    // frontend) — the 304 is decided from the payload AFTER the full load,
    // so the underlying meta+revision read DOES run now.
...
describe('routes/page.detail loader — render waterfall', () => {
  it('starts the music-meta resolution as soon as the preview resolves, without waiting for friends', async () => {
    // The music-meta leg only runs when the body carries a musicPlayer
    // block — seed one so `collectMusicPlayerIds` hands the resolver the
    // player id.
    await seedPage({
      slug: 'links',
      showFriends: true,
      body: lexBody([{ type: 'musicPlayer', playerId: 'abcd1234efgh5678', version: 1 }]),
    })

    // Hold the friends read open: serial composition would only reach the
    // music-meta resolution after the whole first Promise.all settles.
    ...
    await vi.waitFor(() => {
      expect(mocks.getPublicMusicMetasByIds).toHaveBeenCalled()
    })
    release()
    await pending
  })
...
describe('routes/page.detail loader — ETag', () => {
  it('answers 304 when If-None-Match matches, with the payload etag', async () => {
    ...
    expect((second as Response).headers.get('ETag')).toBe(etag)
    // Headless form: the slim DB probe is gone (no database on the
    // frontend) — the 304 is decided from `publicEtag` AFTER the full
    // load, so the underlying meta+revision+image read DOES run now.
