...
const ALLOWED_IS_SAFE_URL_IMPORTERS = ['src/inkling/nodes/base/render-context.ts']

// cleanDOM runs only behind the seam's CALLOUT_HTML_CONFIG unwrap-allowlist
// fallback (plan 040 Step 4 STOP condition).
const ALLOWED_CLEAN_DOM_IMPORTERS = ['src/inkling/nodes/base/render-context.ts']
...
describe('render policy import guard', () => {
  it('no card source under src/nodes/base/nodes imports policy modules directly', () => {
    const nodesDir = join('src', 'inkling', 'nodes', 'base', 'nodes')
  ...
  it('the seam is the only is-safe-url importer', () => {
    const importers = listSourceFiles(join('src', 'inkling'))
      .map((name) => `src/inkling/${name.split(sep).join('/')}`)
