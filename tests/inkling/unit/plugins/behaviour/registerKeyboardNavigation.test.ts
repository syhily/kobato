...
import { DELETE_CARD_COMMAND, SELECT_CARD_COMMAND } from '@/inkling/plugins/behaviour/commands'
import { markEventFromCaptionEditor } from '@/inkling/plugins/behaviour/nested-editor-protocol'
...
describe('registerKeyboardNavigation', () => {
  ...
  it('toggles card edit mode on meta+enter when a card is selected', async () => {
    const { $createCodeBlockNode } = await import('@/inkling/nodes/CodeBlockNode')
    let cardKey = ''
    await updateEditor(editor, () => {
      const root = $getRoot()
