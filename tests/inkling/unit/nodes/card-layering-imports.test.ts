...
// (named by prefix — '@/inkling/nodes/AudioNode' etc.)
const WRAPPER_LAYER_SPECIFIERS = new Set([
  '@/inkling/nodes/cards/card-wrappers',
  '@/inkling/nodes/cards/card-decorate',
  '@/inkling/nodes/cards/card-menus',
  '@/inkling/nodes/cards/card-markdown-transformers',
  '@/inkling/nodes/cards/card-insert-commands',
  '@/inkling/nodes/decorate-card',
])
const FORBIDDEN_PREFIXES = ['@/inkling/components/', '@/inkling/hooks/', '@/inkling/plugins/']
...
function wrapperLayerImportsOf(file: string): string[] {
  return runtimeImportSpecifiers(file).filter(
    (specifier) =>
      ...
      // the shim modules: '@/inkling/nodes/<Card>Node' directly under src/nodes
      ...
      // '@/inkling/nodes/<Card>NodeComponent', '@/inkling/nodes/header/HeaderNodeComponent'
...
const REGISTRY_LAYER_FILES = [
  ...
  'src/inkling/nodes/cards/derive-card-nodes.ts',
]
...
describe('card pipeline layering guard', () => {
  ...
  it('getEditorCardNodes stays off the wrapper node registry', () => {
    ...
    expect(runtimeImportSpecifiers('src/inkling/nodes/cards/editor-card-nodes.ts')).not.toContain(
      '@/inkling/nodes/cards/card-wrappers',
    )
  })
})
