...
import { attachCaptionEditorWithText } from '#/inkling/utils/caption-editor'
...
} from '@/inkling/nodes/GalleryNode'
...
describe('GalleryNode', () => {
  ...
  it('limits images when setting', async () => {
    await updateEditor(editor, () => {
      ...
      node.setImages(images as import('@/inkling/types/gallery').GalleryImage[])
  ...
  it('adds images up to the maximum', async () => {
    await updateEditor(editor, () => {
      const node = $createGalleryNode({})
      node.setImages([{ src: '0' }, { src: '1' }] as import('@/inkling/types/gallery').GalleryImage[])
      node.addImages(
        Array.from({ length: MAX_IMAGES }, (_, i) => ({
          src: String(i + 2),
        })) as import('@/inkling/types/gallery').GalleryImage[],
      )
