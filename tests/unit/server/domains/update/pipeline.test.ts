...
// `fetch` is mocked. Exercises stage → download → verify → chmod → backup →
// swap through `runSelfUpdate`, never its internals.
...
function serveUpdate(binary: string, shaText?: string) {
  const hash = sha256(binary)
  fetchMock.mockImplementation(async (input: unknown) => {
    const url = String(input)
    if (url.endsWith('.sha256')) {
      return new Response(shaText ?? `${hash}  kobato-linux-${process.arch}\n`)
    }
...
describe('update/pipeline', () => {
  ...
  it('downloads, verifies, swaps and keeps a .bak sibling', async () => {
    ...
    expect(urls[0]).toContain(`/releases/download/v9.9.9/kobato-linux-${process.arch}`)
    expect(urls[1]).toBe(`${urls[0]}.sha256`)
  })

  it('aborts on sha256 mismatch without touching the live binary', async () => {
    serveUpdate(NEW_BINARY, `${'0'.repeat(64)}  kobato-linux-${process.arch}\n`)
  ...
  it('restores the live binary when the swap fails after backup', async () => {
    ...
    fetchMock.mockImplementation(async (input: unknown) => {
      const url = String(input)
      if (url.endsWith('.sha256')) {
        // The stage dir exists by now; make it read-only so the swap
        // rename fails after the backup rename succeeded.
        await chmod(stageDir, 0o555)
        return new Response(`${sha256(NEW_BINARY)}  kobato-linux-${process.arch}\n`)
      }
      return new Response(NEW_BINARY)
    })

    await expect(runSelfUpdate({ tagName: 'v9.9.9', execPath })).rejects.toThrow()
  ...
  it('parses the sha256 sidecar format produced by sea.yml', () => {
    ...
    expect(parseSha256Sidecar(`${hash}  kobato-linux-x64\n`)).toBe(hash)
    expect(parseSha256Sidecar(`${hash} kobato-linux-arm64`)).toBe(hash)
    expect(parseSha256Sidecar(`${hash.toUpperCase()}  kobato-linux-x64`)).toBe(hash)
  })
