...
function mockModules() {
  ...
  vi.doMock('@/server/domains/analytics/geoip', () => ({ resetGeoReader }))
...
describe('analytics/geoip-update', () => {
  ...
  describe('fetchLatestGeoipVersion', () => {
    it('parses the version from the jsDelivr package.json', async () => {
      ...
      const { fetchLatestGeoipVersion } = await import('@/server/domains/analytics/geoip-update')
  ...
  describe('runScheduledGeoipUpdate', () => {
    it('never replaces a manually uploaded database', async () => {
      ...
      const { runScheduledGeoipUpdate } = await import('@/server/domains/analytics/geoip-update')
