...
function textBlock(key: string, links: { key: string; href: string }[], text = 'body') {
  return {
    ...
    children: [{ _type: 'span' as const, _key: `${key}-s`, text, marks: links.map((l) => l.key) }],
  }
}
...
describe('webmentions/enqueue.extractExternalLinks', () => {
  ...
  it('caps the extraction at the per-post maximum', () => {
    ...
    expect(extractExternalLinks([textBlock('b1', links)], SITE_HOST)).toHaveLength(MAX_OUTBOUND_LINKS_PER_POST)
  })
})
