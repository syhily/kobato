...
const peekRestoreJobPhase = vi.fn().mockReturnValue({ phase: 'idle', startedAt: '' })
...
describe('/ready endpoint restore-state behaviour', () => {
  ...
  function readyResponse() {
    ...
    if (phase !== 'running') {
      return { status: phase, restore: peekRestoreJobPhase(), code: 503 as const }
    }
    return { status: 'ok', restore: peekRestoreJobPhase(), code: 200 as const }
  }
