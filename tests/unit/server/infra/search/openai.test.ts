...
const openaiState = vi.hoisted(() => ({
  getItemRaw: vi.fn<(db: unknown, key: string) => Promise<unknown>>(),
...
vi.mock('@/server/infra/cache/kv-store', () => ({
  getItemRaw: (db: unknown, key: string) => openaiState.getItemRaw(db, key),
  setItemRaw: (db: unknown, key: string, value: Buffer, opts?: unknown) => openaiState.setItemRaw(db, key, value, opts),
}))
